  ...
  vis
    ...
    .each("end", function () {
      ...
      if (removeOption.checked) {
        ...
        selected_elem.transition().duration(1000).attr("fill", "#111");
      }
        ...
        getTopLeaders(5).then((data) => {
          if (data) {
            for (let i = 0; i < leaderboard.length; i++) {
              leaderboard[
                i
              ].textContent = `${data[i]["name"]} (${data[i]["total"]})`;
            }
          }
        });
...
function incrementNameDB(selectedName) {
  return axios
    .post(api + nameIncrement, { name: selectedName })
    .then((response) => {
